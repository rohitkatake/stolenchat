...
public class ChatServer extends Frame implements Serializable, ActionListener,Runnable,CommonSettings {
	...
	public ChatServer() {
		...
		Label lblTitle = new Label(" CSE CHAT SERVER ",1);
	...
	protected void SendPrivateMessage(String Message , String ToUserName)
	{
		...
		{	if(flag[GetIndexOf(ToUserName)]==1)
			flag[GetIndexOf(ToUserName)]--;
			else
			{flag[GetIndexOf(ToUserName)]--;
			int x=Message.indexOf("Existing AVReceive2");
		    Message=new StringBuffer(Message).replace(x,Message.length(),"Client has stopped receiving").toString();
			}
		}
		if(Message.indexOf("Stop Receiving")!=-1)
		{String s4=Message.substring(0,Message.indexOf(":")+1);
		
		System.out.println(s4);
		flag[GetIndexOf(s4)]=0;
		}
		if(Message.indexOf("Client has Accepted streaming")!=-1)
		{
		Message=Message+"/224.222.222."+(GetIndexOf(ToUserName)+220);
		}
		if(Message.indexOf("Client has accepted and start transmitting")!=-1)
		{
			if(flag[GetIndexOf(ToUserName)]==0)
		     {	 
	          flag[GetIndexOf(ToUserName)]=1;
		     }
		else {
			 flag[GetIndexOf(ToUserName)]++;
			 int x=Message.indexOf("Client has accepted and start transmitting");
		Message=new StringBuffer(Message).replace(x,Message.length(),"Another Client is receiving").toString();
	}
		Message=Message+"/224.222.222."+(GetIndexOf(ToUserName)+200);
		}
